    ...
    for i in range(len(ranges)):
        if (math.isnan(ranges[i]or ranges[i]==0)):
            if (ranges[i]>3.5):
                ranges[i]=3.5
def calculator():
    global dist, x_cor, y_cor, z, pub,ranges,yaw,flag,prev_ang
    z = Twist()
    ...
    if (z.linear.x > 0.1):
        z.linear.x = 0.1
    z.angular.z = 0
    check_angular_speed = 1.8*k_obs_angular+ (0.6*k_g2g_angular*g2g_angular)
    if(abs(prev_ang-check_angular_speed)>0.02):
        z.angular.z = prev_ang+0.3*(check_angular_speed/abs(check_angular_speed))*(prev_ang-check_angular_speed)
    else:
        z.angular.z = 1.8*k_obs_angular+ (0.6*k_g2g_angular*g2g_angular)
    print(prev_ang,check_angular_speed,z.angular.z)
    prev_ang = z.angular.z
