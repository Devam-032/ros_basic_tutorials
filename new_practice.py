...
def callback(data):
    ...
    # pub.publish(z)
    # rate.sleep()
    """This is the 3rd logic"""
    a,b=[],[]
    a=min_array_size[2:0:-1]+[min_array_size[0]]
    b=min_array_size[9:12]
    print(len(b))
    k=1.25
    ...
    c=sum(a)+sum(b)
    print(c)
    z.linear.x=abs(3-(math.sqrt(c*c)))/30
    z.angular.z=(c/20)
    pub.publish(z)
def min_range_index(ranges):
    ...
    b=[min(y[0]),min(y[1]),min(y[2]),min(y[3]),min(y[4]),min(y[5]),min(y[6]),min(y[7]),min(y[8]),min(y[9]),min(y[10]),min(y[11])]
    return (min(ranges), ranges.index(min(ranges)),len(b),b)
